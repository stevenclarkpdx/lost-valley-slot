...
function App() {
  ...
  const handleSpin = () => {
    if (feature || balance < bet || isReeling || triggerTransition) return
    spinTimers.current.forEach((timer) => window.clearTimeout(timer))
    spinTimers.current = []
          ...
          {!feature && (
            <div className="controls">
              ...
              <button
                ...
                onClick={handleSpin}
                disabled={balance < bet || isReeling || triggerTransition}
              >
...
function BaseGame({
  ...
  return (
    <div className={`base-game ${isReeling ? 'reels-active' : ''}`}>
      <div className="game-label">
        <span>Expedition grid</span>
        <span>{spin.footprintCount}/3 evidence found</span>
      </div>
      <div className="expedition-playfield">
                ...
                <div
                  className={`symbol reel-${columnIndex} ${
                    ...
                    FIELD_NOTE_SYMBOLS.includes(symbol as (typeof FIELD_NOTE_SYMBOLS)[number])
                      ? 'evidence'
                      : ''
                  } ${winning && !isReeling ? 'winning' : ''} ${
                    isReeling && columnIndex >= settledReels ? 'reel-spinning' : ''
                  } ${isReeling && columnIndex < settledReels ? 'reel-settled' : ''} ${
                    triggerTransition && symbol === 'footprint' ? 'trigger-footprint' : ''
                  }`}
...
function FieldNotesPanel({ spin }: { spin: BaseSpinResult }) {
  const found = new Set(spin.fieldNotes.uniqueEvidence)
  return (
    <aside
      className={`field-notes ${spin.fieldNotes.bonus > 0 ? 'bonus-hit' : ''}`}
      key={spin.board.flat().join('-')}
    >
      <div className="field-notes-heading">
        <span className="eyebrow">Field Notes</span>
        <strong>{spin.fieldNotes.uniqueEvidence.length}/5</strong>
      </div>
